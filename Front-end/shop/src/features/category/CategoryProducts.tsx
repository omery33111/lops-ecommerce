...
import { deleteCategoryAsync, getCategoryProductsAsync, getSingleCategoryAsync, patchCategoryAsync, selectCategory, selectCategoryProducts } from './categorySlice';
...
const CategoryProducts = () => {
  ...
  useEffect(() => {
    ...
    const onSubmit = async (event: any) => {
      ...
      dispatch(patchCategoryAsync({categoryData: formData, id: single_category.id}));
      setCategoryName("");
      setTimeout(() => {
    ...
    return (
      ...
      {storedIsStaff ? (<div>
        ...
        <Button variant="warning" onClick={() => setShowForm2(!showForm2)} style = {{position: "absolute", transform: " translateX(1250px) translateY(-4px) "}}>
      ...
      {showForm2 && (
        ...
        <Form onSubmit={onSubmit}>
           <Form.Group controlId="formCategory" style = {{position: "absolute", transform: " translateX(1020px) translateY(1px) "}}>
             <Form.Control
              type="text"
          ...
          <Button variant="warning" type="submit" style = {{position: "absolute", transform: " translateX(965px) translateY(1px) "}}>
            <BsCheckLg />
          </Button>
        </Form>


        <Button onClick = {handleShow} variant = "danger" style = {{position: "absolute", transform: " translateX(870px) translateY(-6px) "}}> <h4><BsTrash /></h4> </Button>
        <Modal show={showModal} onHide={handleClose} animation={false}>
                <Modal.Header closeButton>
                  <Modal.Title><BsTrash /> Delete Warning </Modal.Title>
                </Modal.Header>
                <Modal.Body>Are you sure you want to delete "{singleCategory.category_name}"?</Modal.Body>
                <Modal.Footer>
                  <Button variant="secondary" onClick={handleClose}>
                    Close
                  </Button>
                  <Link to = "/">
                  <Button variant="danger" onClick = {() => singleCategory.id && dispatch(deleteCategoryAsync(singleCategory.id))}>
                    Delete
                  </Button>
                  </Link>
                </Modal.Footer>
