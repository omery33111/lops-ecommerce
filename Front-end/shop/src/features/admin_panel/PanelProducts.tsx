...
const PanelProducts = () => {
  ...
  <Button variant = "warning" style={{position: "absolute", transform: " translateX(835px) translateY(-50px)"}}>
  <Link style = {{textDecoration: "none", color: "black"}} to = {`/admin_panel/post_product/`}>
    Add Product
   </Link>
   </Button>
  ...
  <tr key={product.id}>
    ...
    <td style={{ textAlign: "center", verticalAlign: "middle", height: "100px"}}>{product.category}</td>
    <td style={{ textAlign: "center", verticalAlign: "middle", height: "100px"}}>
        <Button style={{ color: "blue", border: "none", background: "none"}}
         onClick={() => navigate(`/admin_panel/update_product/${product.id}`)}
         >
        Edit
        </Button>
        |
        <Button style={{ color: "red", border: "none", background: "none"}}
         onClick={() => dispatch(deleteProductAsync(Number(product.id)))}>
          Delete
          </Button>
         </td>
  </tr>
        ...
        {products.length >= offset + limit && (
          <div style={{ position: "absolute", transform: "translateX(480px) translateY(30px)" }}>
            {showSpinner &&
                <Stack>
