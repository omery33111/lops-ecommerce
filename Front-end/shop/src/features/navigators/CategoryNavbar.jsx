...
const isMobile = window.innerWidth <= 768;
  ...
  return (
    <div>
      ...
      <Navbar  className={`${!isScrolling ? 'scrolling' : 'fixed-top'}`} style={{ width: "100%", backgroundColor: "#F0EAD6" }}>

      
        <Nav>
        {storedIsStaff && !isMobile ? (<Nav className="me-start">
      ...
      {isScrolling? (
        <>
        {!isMobile && (
          <Nav.Link style = {{width: "150px", color: "black", position: "absolute", transform: " translateX(150px) translateY(1px) "}}>
        ...
        <Container style={{ display: "flex", justifyContent: "center", alignItems: "center", width: "100%" }}>
          {categories.map((category) => (
            <div key={category.id} style={{ margin: isMobile ? "0 5px" : "0 20px" }}>
              <Nav>
                <Nav.Link as={Link} to={`/category/category_products/${category.id}/`} className="text-black">
                  <b style = {{fontSize: isMobile && "0.8rem"}}>{category.category_name}</b>
                </Nav.Link>
              </Nav>
            </div>
          ))}
        ...
        {!isMobile && (
            ...
            {myCart.length === 0 ? (
              ...
              <div style={{ color: 'white', position: "absolute", transform: 'translateX(-185px) translateY(-13px)'}}>
              ...
              <OverlayTrigger
                ...
                overlay={
                  ...
                  <Container className="p-3">
                    ...
                    <Card className = "card border-0 rounded-0" style={{ width: '23%', position: "absolute", top: 0, right: 140, backgroundColor: "#F0EAD6"}}>
                      <Card.Header style={{ backgroundColor: "#F0EAD6", color: 'black', textAlign: 'center' }}>SHOPPING CART</Card.Header>
